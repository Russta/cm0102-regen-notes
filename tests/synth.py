...
from cm0102_regen_notes.records import STAFF_RECORD_LEN, PLAYER_RECORD_LEN, S_PLAYER_FK, S_NATION
...
def staff_record(staff_id: int, first: int, second: int, common: int,
                 player_fk: int, nation: int = 0) -> bytes:
...
def make_world():
    ...
    # now: slot 2 (Zidane) has been regenerated into "Reginald Bloggs"
    staff = [
        staff_record(101, 1, 1, 0, player_fk=0, nation=7),
        staff_record(102, 2, 2, 0, player_fk=1, nation=7),
        staff_record(103, 4, 4, 0, player_fk=2, nation=1),  # regen in Zidane's slot
        staff_record(104, 4, 4, 0, player_fk=3, nation=9),
    ]
    players = [
